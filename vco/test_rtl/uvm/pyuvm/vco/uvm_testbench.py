...
class vcoAgent(uvm_agent):
    def build_phase(self):
        self.monitor = vcoMonitor.create("monitor", self)
        self.driver = vcoDriver.create("driver", self)
        self.sequencer = uvm_sequencer("sequencer", self)
        
    def connect_phase(self):
        self.driver.seq_item_port.connect(self.sequencer.seq_item_export)

class vcoScoreboard(uvm_scoreboard):
    def build_phase(self):
...
class vcoEnv(uvm_env):
    def build_phase(self):
        self.agent = vcoAgent.create("agent", self)
        self.scoreboard = vcoScoreboard.create("scoreboard", self)
...
class vcoTest(uvm_test):
    def build_phase(self):
        self.env = vcoEnv.create("env", self)
...
async def run_test(dut):
    await uvm_root().run_test("vcoTest")