    ...
    # Append `define MCY after `timescale 1ps/1ps to step1.v using awk
    awk '1;/`timescale 1ps\/1ps/{print "`define MCY"}' step1.v > template_temp.v
    mv template_temp.v step1.v

    #replace the orginal testbench file with the temp file.
    mv template_temp.v step1.v

    # Copy step1 here
    cp ${PWD}/../../simulation/icarus/step1/testbench.v .
